...
import {
  VerificationTask, ShiftedResiCumOfficeReportData, AddressLocatable, AddressRating, ResiCumOfficeStatus,
...
const ShiftedResiCumOfficeForm: React.FC<ShiftedResiCumOfficeFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateShiftedResiCumOfficeReport, updateTaskStatus, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.shiftedResiCumOfficeReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateShiftedResiCumOfficeReport(taskData.id, updates);
  };
