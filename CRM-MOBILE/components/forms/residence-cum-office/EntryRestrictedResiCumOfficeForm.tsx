...
import {
  VerificationTask, EntryRestrictedResiCumOfficeReportData, AddressLocatable, AddressRating, MetPersonErt,
...
const EntryRestrictedResiCumOfficeForm: React.FC<EntryRestrictedResiCumOfficeFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateEntryRestrictedResiCumOfficeReport, updateTaskStatus, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.entryRestrictedResiCumOfficeReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateEntryRestrictedResiCumOfficeReport(taskData.id, updates);
  };
