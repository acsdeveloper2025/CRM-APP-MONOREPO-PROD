...
import {
  VerificationTask, UntraceableResidenceReportData, CallRemarkUntraceable, LocalityType, DominatedArea, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
const UntraceableResidenceForm: React.FC<UntraceableResidenceFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateUntraceableResidenceReport, toggleSaveTask, fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.untraceableResidenceReport;
  ...
  const isFormValid = useMemo(() => {
    ...
    const baseFields: (keyof UntraceableResidenceReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<UntraceableResidenceReportData> = { [name]: processedValue };
    updateUntraceableResidenceReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.RESIDENCE_UNTRACEABLE}
      formData={report}
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Untraceable Residence Report</h3>
...
export default UntraceableResidenceForm;