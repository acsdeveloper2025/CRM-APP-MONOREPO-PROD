...
import {
  VerificationTask, NspBusinessReportData, AddressLocatable, AddressRating, OfficeStatusOffice, BusinessExistence,
...
const NspBusinessForm: React.FC<NspBusinessFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateNspBusinessReport, toggleSaveTask, fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.nspBusinessReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateNspBusinessReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.BUSINESS_NSP}
      formData={report}
