...
import {
  VerificationTask, ShiftedBusinessReportData, AddressLocatable, AddressRating, OfficeStatusOffice, DesignationShiftedOffice,
...
interface ShiftedBusinessFormProps {
...
const ShiftedBusinessForm: React.FC<ShiftedBusinessFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateShiftedBusinessReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.shiftedBusinessReport;
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof ShiftedBusinessReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof ShiftedBusinessReportData)[] = [
    ...
    if (report.officeStatus === OfficeStatusOffice.Opened) {
        const openedOnlyFields: (keyof ShiftedBusinessReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<ShiftedBusinessReportData> = { [name]: processedValue };
    updateShiftedBusinessReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.BUSINESS_SHIFTED}
      formData={report}
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Shifted Business Report</h3>
      ...
      <div className="p-4 bg-gray-900/50 rounded-lg space-y-4 border border-dark-border">
        ...
        {/* Old Office Shifted Period - Always visible */}
...
export default ShiftedBusinessForm;