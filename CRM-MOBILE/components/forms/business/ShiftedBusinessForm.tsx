...
import {
  Case, ShiftedBusinessReportData, AddressLocatable, AddressRating, OfficeStatusOffice, DesignationShiftedOffice,
...
const ShiftedBusinessForm: React.FC<ShiftedBusinessFormProps> = ({ caseData }) => {
  const { updateShiftedBusinessReport, updateCaseStatus, toggleSaveCase } = useCases();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const report = caseData.shiftedBusinessReport;
