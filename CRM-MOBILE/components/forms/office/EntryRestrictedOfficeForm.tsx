...
const EntryRestrictedOfficeForm: React.FC<EntryRestrictedOfficeFormProps> = ({ caseData }) => {
  const { updateEntryRestrictedOfficeReport, updateCaseStatus, toggleSaveCase } = useCases();
  ...
  if (!report) {
    return <p className="text-medium-text">No Entry Restricted Office report data available.</p>;
  }
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof EntryRestrictedOfficeReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof EntryRestrictedOfficeReportData)[] = [
        'addressLocatable', 'addressRating', 'metPerson', 'nameOfMetPerson', 'metPersonConfirmation',
        'officeStatus', 'locality', 'addressStructure', 'officeExistFloor', 'addressStructureColor',
  ...
  const options = useMemo(() => ({
    ...
    officeStatus: getEnumOptions(OfficeStatusErtOffice),
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Entry Restricted Office Report</h3>
...
export default EntryRestrictedOfficeForm;