...
import {
  VerificationTask, ShiftedOfficeReportData, AddressLocatable, AddressRating, OfficeStatusOffice, DesignationShiftedOffice,
  SightStatus, TPCMetPerson, TPCConfirmation, LocalityTypeResiCumOffice, PoliticalConnection, DominatedArea,
...
const ShiftedOfficeForm: React.FC<ShiftedOfficeFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateShiftedOfficeReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.shiftedOfficeReport;
  ...
  if (!report) {
    return <p className="text-medium-text">No Shifted Office report data available.</p>;
  }
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof ShiftedOfficeReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof ShiftedOfficeReportData)[] = [
        ...
        'currentCompanyName', 'currentCompanyPeriod', 'oldOfficeShiftedPeriod', 'companyNamePlateStatus'
    ];
    ...
    if (report.officeStatus === OfficeStatusOffice.Opened) {
        const openedOnlyFields: (keyof ShiftedOfficeReportData)[] = [
            'metPerson', 'designation', 'officeApproxArea'
        ];
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<ShiftedOfficeReportData> = { [name]: processedValue };
    updateShiftedOfficeReport(taskData.id, updates);
  };
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Shifted Office Report</h3>
                ...
                onConfirm={async () => {
                    ...
                    try {
                        ...
                        console.log('🔍 ShiftedOfficeForm submission debug:', {
                            caseVerificationOutcome: taskData.verificationOutcome,
                            reportOutcome: report.outcome,
                            finalOutcome: taskData.verificationOutcome || report.outcome || 'Shifted & Door Lock'
                        });
...
export default ShiftedOfficeForm;