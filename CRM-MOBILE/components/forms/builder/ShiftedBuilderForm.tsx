...
const ShiftedBuilderForm: React.FC<ShiftedBuilderFormProps> = ({ caseData }) => {
  const { updateShiftedBuilderReport, updateCaseStatus, toggleSaveCase } = useCases();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const report = caseData.shiftedBuilderReport;
  ...
  if (!report) {
    return <p className="text-medium-text">No Shifted Builder report data available.</p>;
  }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<ShiftedBuilderReportData> = { [name]: processedValue };
    updateShiftedBuilderReport(caseData.id, updates);
  };
  ...
  return (
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Shifted Builder Report</h3>
