...
import {
  VerificationTask, NspBuilderReportData, AddressLocatable, AddressRating, OfficeStatusOffice, BusinessExistence,
...
const NspBuilderForm: React.FC<NspBuilderFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateNspBuilderReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.nspBuilderReport;
  ...
  const isFormValid = useMemo(() => {
    ...
    if (report.officeStatus === OfficeStatusOffice.Opened) {
        const openedOnlyFields: (keyof NspBuilderReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateNspBuilderReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.BUILDER_NSP}
      formData={report}
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">NSP Builder Report</h3>
