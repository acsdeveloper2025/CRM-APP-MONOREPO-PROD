...
import {
  VerificationTask, UntraceableBuilderReportData, CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
interface UntraceableBuilderFormProps {
...
const UntraceableBuilderForm: React.FC<UntraceableBuilderFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateUntraceableBuilderReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.untraceableBuilderReport;
  ...
  if (!report) {
    return <p className="text-medium-text">No Untraceable Builder report data available.</p>;
  }
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof UntraceableBuilderReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof UntraceableBuilderReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<UntraceableBuilderReportData> = { [name]: processedValue };
    updateUntraceableBuilderReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.BUILDER_UNTRACEABLE}
      formData={report}
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Untraceable Builder Report</h3>
...
export default UntraceableBuilderForm;