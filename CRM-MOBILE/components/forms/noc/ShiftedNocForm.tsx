...
import {
  VerificationTask, ShiftedNocReportData, AddressLocatable, AddressRating, OfficeStatusOffice, DesignationNoc, SightStatus,
...
const ShiftedNocForm: React.FC<ShiftedNocFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateShiftedNocReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.shiftedNocReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateShiftedNocReport(taskData.id, updates);
  };
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.NOC_SHIFTED}
      formData={report}
    ...
    >
      ...
      {/* Conditional Personal Details Section - Only show when office is open */}
      {report.officeStatus === OfficeStatusOffice.Opened && (
        <div className="p-4 bg-yellow-900/20 rounded-lg space-y-4 border border-yellow-600/30">
          <h4 className="font-semibold text-yellow-400">Personal Details (Office Open)</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
