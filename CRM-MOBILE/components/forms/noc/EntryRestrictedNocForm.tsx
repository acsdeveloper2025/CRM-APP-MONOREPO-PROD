...
import {
  Case, EntryRestrictedNocReportData, AddressLocatable, AddressRating, MetPersonErt, TPCConfirmation,
  OfficeStatusErtNoc, LocalityTypeResiCumOffice, PoliticalConnection, DominatedArea, FeedbackFromNeighbour, FinalStatus, CaseStatus, CapturedImage
} from '../../../types';
...
const EntryRestrictedNocForm: React.FC<EntryRestrictedNocFormProps> = ({ caseData }) => {
  const { updateEntryRestrictedNocReport, updateCaseStatus, toggleSaveCase } = useCases();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const report = caseData.entryRestrictedNocReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateEntryRestrictedNocReport(caseData.id, updates);
  };
