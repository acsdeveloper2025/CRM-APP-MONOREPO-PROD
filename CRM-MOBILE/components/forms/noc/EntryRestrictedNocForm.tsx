...
import {
  VerificationTask, EntryRestrictedNocReportData, AddressLocatable, AddressRating, MetPersonErt, TPCConfirmation,
  OfficeStatusErtNoc, LocalityTypeResiCumOffice, PoliticalConnection, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
interface EntryRestrictedNocFormProps {
...
const EntryRestrictedNocForm: React.FC<EntryRestrictedNocFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateEntryRestrictedNocReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.entryRestrictedNocReport;
  ...
  if (!report) {
    return <p className="text-medium-text">No Entry Restricted NOC report data available.</p>;
  }
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof EntryRestrictedNocReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof EntryRestrictedNocReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<EntryRestrictedNocReportData> = { [name]: processedValue };
    updateEntryRestrictedNocReport(taskData.id, updates);
  };
  ...
  const options = useMemo(() => ({
    ...
    officeStatus: getEnumOptions(OfficeStatusErtNoc),
  ...
  return (
    <AutoSaveFormWrapper
      taskId={taskData.id}
      formType={FORM_TYPES.NOC_ENTRY_RESTRICTED}
      formData={report}
    ...
    >
      ...
      <h3 className="text-lg font-semibold text-brand-primary">Entry Restricted NOC Report</h3>
...
export default EntryRestrictedNocForm;