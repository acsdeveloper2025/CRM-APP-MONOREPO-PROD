...
import {
  VerificationTask, UntraceableNocReportData, CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
const UntraceableNocForm: React.FC<UntraceableNocFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateUntraceableNocReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.untraceableNocReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateUntraceableNocReport(taskData.id, updates);
  };
