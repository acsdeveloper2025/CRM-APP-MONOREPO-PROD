...
import {
  Case, UntraceablePropertyApfReportData, CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea, FinalStatus, CaseStatus, CapturedImage
} from '../../../types';
...
interface UntraceablePropertyApfFormProps {
  caseData: Case;
}
...
const UntraceablePropertyApfForm: React.FC<UntraceablePropertyApfFormProps> = ({ caseData }) => {
  const { updateUntraceablePropertyApfReport, updateCaseStatus, toggleSaveCase } = useCases();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const report = caseData.untraceablePropertyApfReport;
  ...
  const isFormValid = useMemo(() => {
    ...
    const checkFields = (fields: (keyof UntraceablePropertyApfReportData)[]) => fields.every(field => {
        const value = report[field];
        return value !== null && value !== undefined && value !== '';
    });

    const baseFields: (keyof UntraceablePropertyApfReportData)[] = [
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    const updates: Partial<UntraceablePropertyApfReportData> = { [name]: processedValue };
    ...
    >
      ...
      <div className="p-4 bg-gray-900/50 rounded-lg space-y-4 border border-dark-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <FormField label="Contact Person" id="metPerson" name="metPerson" value={report.metPerson} onChange={handleChange} disabled={isReadOnly} />
                ...
                onConfirm={async () => {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            setIsConfirmModalOpen(false);
                            console.log('✅ Property apf verification submitted successfully');
                        } else {
...
export default UntraceablePropertyApfForm;