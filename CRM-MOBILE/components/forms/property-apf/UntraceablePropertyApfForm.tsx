...
import {
  VerificationTask, UntraceablePropertyApfReportData, CallRemarkUntraceable, LocalityTypeResiCumOffice, DominatedArea, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
const UntraceablePropertyApfForm: React.FC<UntraceablePropertyApfFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateUntraceablePropertyApfReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.untraceablePropertyApfReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateUntraceablePropertyApfReport(taskData.id, updates);
  };
