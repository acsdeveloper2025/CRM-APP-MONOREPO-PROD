...
import {
  VerificationTask, EntryRestrictedDsaReportData, AddressLocatable, AddressRating, MetPersonErt, TPCConfirmation,
  OfficeStatusErtDsa, LocalityTypeResiCumOffice, PoliticalConnection, DominatedArea, FeedbackFromNeighbour, FinalStatus, TaskStatus, CapturedImage
} from '../../../types';
...
const EntryRestrictedDsaForm: React.FC<EntryRestrictedDsaFormProps> = ({ taskData }) => {
  const navigate = useNavigate();
  const { updateEntryRestrictedDsaReport, toggleSaveTask , fetchTasks } = useTasks();
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submissionError, setSubmissionError] = useState<string | null>(null);
  const [submissionSuccess, setSubmissionSuccess] = useState(false);
  const report = taskData.entryRestrictedDsaReport;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    updateEntryRestrictedDsaReport(taskData.id, updates);
  };
