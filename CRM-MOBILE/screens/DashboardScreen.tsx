...
const DashboardScreen: React.FC = () => {
  ...
  const assignedCount = cases.filter(c => (c.taskStatus || c.status) === TaskStatus.Assigned).length;
  const inProgressCount = cases.filter(c => (c.taskStatus || c.status) === TaskStatus.InProgress).length;
  const completedCount = cases.filter(c => (c.taskStatus || c.status) === TaskStatus.Completed).length;
