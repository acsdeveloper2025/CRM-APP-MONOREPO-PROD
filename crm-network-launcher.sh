...
generate_config_from_template() {
    ...
    local output_file="$2"
    local domain_name="${DOMAIN_NAME:-example.com}"
    local db_host="${DB_HOST:-localhost}"
    local db_port="${DB_PORT:-5432}"
    local db_name="${DB_NAME:-acs_db}"
    local db_user="${DB_USER:-example_db_user}"
    local db_password="${DB_PASSWORD:-example_db_password}"
...
if [ -f "$BACKEND_TEMPLATE" ] && [ ! -f "$BACKEND_ENV" ]; then
    ...
    if [ "$USE_STATIC_IP" = true ]; then
        if [ "$IP_CHOICE" = "3" ]; then
            ...
            CORS_ORIGINS="http://localhost:5173,http://localhost:5180,http://127.0.0.1:5173,http://127.0.0.1:5180,http://$LOCAL_NETWORK_IP:5173,http://$LOCAL_NETWORK_IP:5180,http://$STATIC_IP:5173,http://$STATIC_IP:5180,http://$STATIC_IP,https://example.com,capacitor://localhost,http://capacitor.localhost"
            WS_CORS_ORIGINS="http://localhost:5173,http://localhost:5180,http://127.0.0.1:5173,http://127.0.0.1:5180,http://$LOCAL_NETWORK_IP:5173,http://$LOCAL_NETWORK_IP:5180,http://$STATIC_IP:5173,http://$STATIC_IP:5180,http://$STATIC_IP,https://example.com,capacitor://localhost,http://capacitor.localhost"
        else
            # Static IP only
            CORS_ORIGINS="http://localhost:5173,http://localhost:5180,http://127.0.0.1:5173,http://127.0.0.1:5180,http://$STATIC_IP:5173,http://$STATIC_IP:5180,http://$STATIC_IP,https://example.com,capacitor://localhost,http://capacitor.localhost"
            WS_CORS_ORIGINS="http://localhost:5173,http://localhost:5180,http://127.0.0.1:5173,http://127.0.0.1:5180,http://$STATIC_IP:5173,http://$STATIC_IP:5180,http://$STATIC_IP,https://example.com,capacitor://localhost,http://capacitor.localhost"
        fi
    else
        # Local network only
