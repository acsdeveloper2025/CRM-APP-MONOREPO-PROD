...
install_postgresql() {
    ...
    sudo -u postgres psql -c "CREATE USER acs_user WITH PASSWORD 'acs_password';" 2>/dev/null || print_info "User acs_user already exists"
    sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE acs_db TO acs_user;" 2>/dev/null || true
    sudo -u postgres psql -c "ALTER USER acs_user CREATEDB;" 2>/dev/null || true
