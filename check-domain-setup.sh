...
# 🌐 Domain Setup Status Checker for crm.allcheckservices.com
...
if command -v nslookup >/dev/null 2>&1; then
    if nslookup crm.allcheckservices.com | grep -q "103.14.234.36"; then
        print_status "DNS correctly points to 103.14.234.36"
    else
        print_warning "DNS not pointing to 103.14.234.36 or not propagated yet"
        print_info "Current DNS resolution:"
        nslookup crm.allcheckservices.com | grep -A 2 "Name:"
    fi
else
...
if systemctl is-active --quiet nginx; then
    ...
    if [ -L "/etc/nginx/sites-enabled/crm.allcheckservices.com" ]; then
...
if [ -f "/etc/letsencrypt/live/crm.allcheckservices.com/fullchain.pem" ]; then
    ...
    cert_expiry=$(sudo openssl x509 -enddate -noout -in /etc/letsencrypt/live/crm.allcheckservices.com/fullchain.pem | cut -d= -f2)
    print_info "Certificate expires: $cert_expiry"
    
    # Check if certificate is valid for more than 30 days
    if sudo openssl x509 -checkend 2592000 -noout -in /etc/letsencrypt/live/crm.allcheckservices.com/fullchain.pem >/dev/null 2>&1; then
...
if command -v curl >/dev/null 2>&1; then
    http_status=$(curl -s -o /dev/null -w "%{http_code}" -L --max-time 10 http://crm.allcheckservices.com 2>/dev/null || echo "000")
    ...
    # Test HTTPS access
    https_status=$(curl -s -o /dev/null -w "%{http_code}" -k --max-time 10 https://crm.allcheckservices.com 2>/dev/null || echo "000")
