# 🌐 Domain Setup Guide: example.com

This guide explains how to set up the domain `example.com` to redirect to your CRM server at IP `PUBLIC_STATIC_IP`.

## 📋 **Prerequisites**

- Domain `allcheckservices.com` registered and accessible
- Server running at IP `PUBLIC_STATIC_IP`
...
# Expected result: PUBLIC_STATIC_IP
```
...
- `example.com` → Uses HTTPS with domain
- `www.example.com` → Uses HTTPS with domain
- `PUBLIC_STATIC_IP` → Uses HTTP with static IP
...
- **Frontend**: `http://PUBLIC_STATIC_IP:5173`
- **Mobile**: `http://PUBLIC_STATIC_IP:5180`
- **API**: `http://PUBLIC_STATIC_IP:3000`
...
wscat -c wss://example.com/socket.io/
```
...
**🎉 Your CRM system will be accessible worldwide via `https://example.com`!**
