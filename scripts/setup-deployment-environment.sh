...
CREATE DATABASE acs_db OWNER example_db_user;
GRANT ALL PRIVILEGES ON DATABASE acs_db TO example_db_user;
\q
EOF
...
setup_ssl_certificates() {
    ...
    if sudo certbot certificates 2>/dev/null | grep -q "example.com"; then
        print_status "SSL certificates already exist for example.com"
        return 0
    fi
    
    print_warning "SSL certificates not found. You need to set them up manually:"
    print_info "Run: sudo certbot --nginx -d example.com"
...
server {
    listen 80;
    server_name example.com SERVER_IP;
...
verify_setup() {
    ...
    if [ "$checks_passed" = true ]; then
        ...
        print_info "2. Set up SSL certificates: sudo certbot --nginx -d example.com"
