...
LOG_FILE="/home/admin1/logs/rollback.log"
...
restore_database() {
    ...
    PGPASSWORD=acs_password pg_dump -h localhost -U acs_user -d acs_db > "$BACKUP_DIR/pre-rollback-db-$(date +%Y%m%d-%H%M%S).sql"
    
    print_info "Restoring database from backup..."
    
    # Drop and recreate database
    PGPASSWORD=acs_password psql -h localhost -U acs_user -d postgres -c "DROP DATABASE IF EXISTS acs_db;"
    PGPASSWORD=acs_password psql -h localhost -U acs_user -d postgres -c "CREATE DATABASE acs_db;"
    
    # Restore from backup
    PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db < "$db_backup"
...
verify_rollback() {
    ...
    if [ -f "/tmp/health-check.sh" ]; then
        ...
        if curl -s -f "https://crm.allcheckservices.com/health" >/dev/null; then
