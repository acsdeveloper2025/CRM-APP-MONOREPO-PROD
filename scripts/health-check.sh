...
check_database() {
    ...
    if PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -c "SELECT 1;" >/dev/null 2>&1; then
        ...
        local user_count=$(PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -t -c "SELECT COUNT(*) FROM users;" 2>/dev/null | xargs)
...
check_http_endpoints() {
    ...
    local auth_response=$(curl -s -w "%{http_code}" -o /dev/null --max-time $TIMEOUT \
        ...
        "https://crm.allcheckservices.com/api/auth/login" 2>/dev/null || echo "000")
