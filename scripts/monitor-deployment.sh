...
check_database() {
    if PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -c "SELECT 1;" >/dev/null 2>&1; then
        DATABASE_FAILURES=0
        return 0
    else
...
monitor_backend() {
    ...
    # Check HTTP endpoint
    elif ! check_endpoint "https://crm.allcheckservices.com/api/health"; then
...
monitor_frontend() {
    ...
    # Check HTTP endpoint
    elif ! check_endpoint "https://crm.allcheckservices.com/"; then
        status="❌ ENDPOINT DOWN"
...
monitor_database() {
    ...
    if ! check_database; then
        ...
        # Get database info
        local db_size=$(PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -t -c "SELECT pg_size_pretty(pg_database_size('acs_db'));" 2>/dev/null | xargs || echo "N/A")
        local active_connections=$(PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -t -c "SELECT count(*) FROM pg_stat_activity WHERE state = 'active';" 2>/dev/null | xargs || echo "N/A")
...
monitor_ssl() {
    ...
    if command -v openssl >/dev/null 2>&1; then
        local cert_info=$(echo | openssl s_client -servername crm.allcheckservices.com -connect crm.allcheckservices.com:443 2>/dev/null | openssl x509 -noout -dates 2>/dev/null)
    ...
    cat > "$report_file" << EOF
{
    ...
    "services": {
        "backend": {
            ...
            "endpoint_responding": $(check_endpoint "https://crm.allcheckservices.com/api/health" && echo "true" || echo "false"),
