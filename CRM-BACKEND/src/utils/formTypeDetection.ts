...
function detectByFieldIndicators(
  ...
  const detectedType =
    Object.keys(scores).find(key => scores[key as keyof typeof scores] === maxScore) || 'POSITIVE';
...
function detectByPatterns(formData: DetectedFormData, _verificationType: string): FormTypeResult {
  ...
  if (
    formData.callRemark?.toLowerCase().includes('not reachable') ||
    (formData.contactPerson && !formData.metPersonName) ||
    formData.phoneStatus?.toLowerCase().includes('switched off')
  ) {
    formType = 'UNTRACEABLE';
    confidence = 85;
  } else if (
    ...
    formData.stayingStatus?.toLowerCase().includes('temporary') ||
    formData.businessStatus?.toLowerCase().includes('closed')
  ) {
    formType = 'NSP';
    confidence = 70;
  }
...
export function analyzeFormTypeDetection(
  ...
  // Analyze patterns
  if (formData.callRemark?.toLowerCase().includes('not reachable')) {
