...
export class EmailDeliveryService {
  ...
  // Using any due to CI type resolution issues with nodemailer
  ...
  public async sendEmail(options: EmailOptions): Promise<EmailResult> {
    try {
      ...
      logger.info(`Email sent successfully to ${recipient}. Message ID: ${msgId}`);
...
This email was automatically generated by the CRM Analytics System.
  ...
  private generateReportSummaryHTML(reportData: ReportData): string {
  ...
  private generateReportSummaryText(reportData: ReportData): string {
    if (!reportData.summary) {
      return '';
    }
    ...
    const summaryText = Object.entries(reportData.summary || {})
      .map(
        ([key, value]) =>
          `${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}: ${this.formatSummaryValue(value)}`
      )
      .join('\n');
