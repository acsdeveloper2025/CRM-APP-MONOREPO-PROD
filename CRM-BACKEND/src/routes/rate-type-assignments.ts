...
const combinationValidation = [
  query('clientId').isInt({ min: 1 }).withMessage('Client ID must be a valid integer'),
  query('productId').isInt({ min: 1 }).withMessage('Product ID must be a valid integer'),
  query('verificationTypeId')
    .isInt({ min: 1 })
    .withMessage('Verification Type ID must be a valid integer'),
];
...
const bulkAssignValidation = [
  ...
  body('verificationTypeId')
    .isInt({ min: 1 })
    .withMessage('Verification Type ID must be a valid integer'),
  body('rateTypeIds').isArray().withMessage('Rate Type IDs must be an array'),
  body('rateTypeIds.*').isInt({ min: 1 }).withMessage('Each Rate Type ID must be a valid integer'),
];
