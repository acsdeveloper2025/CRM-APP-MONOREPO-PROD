...
-- transaction. On the current small dataset these in-tx builds are instant.
-- When these tables are large, pre-create each index CONCURRENTLY out-of-band
-- BEFORE the deploy so the in-tx CREATE … IF NOT EXISTS below is a no-op (zero
-- lock). Out-of-band form: same statements with `CREATE INDEX CONCURRENTLY`.
--
...
CREATE INDEX IF NOT EXISTS idx_commission_calculations_created_at
  ON commission_calculations (created_at DESC);
-- Status-filtered stats/summary aggregates + pivot status<>'REJECTED'.
CREATE INDEX IF NOT EXISTS idx_commission_calculations_status_created
...
CREATE INDEX IF NOT EXISTS idx_cases_customer_phone_trgm
...
CREATE INDEX IF NOT EXISTS idx_vt_completed_at_completed
  ON verification_tasks (completed_at) WHERE status = 'COMPLETED';
CREATE INDEX IF NOT EXISTS idx_vt_started_at_in_progress
...
CREATE INDEX IF NOT EXISTS idx_vt_parent_status
...
CREATE INDEX IF NOT EXISTS idx_vt_assigned_geo
...
CREATE INDEX IF NOT EXISTS idx_form_submissions_submitter_recent
...
CREATE INDEX IF NOT EXISTS idx_tfs_task_recent
