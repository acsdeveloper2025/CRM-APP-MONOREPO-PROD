...
**Command:**
```bash
PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -c "SELECT * FROM schema_migrations ORDER BY executed_at DESC LIMIT 3;"
```
...
**Command:**
```bash
PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -c "SELECT * FROM migration_test;"
```
...
2. **Verify Migration Executed:**
   ```bash
   PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db -c "SELECT * FROM schema_migrations ORDER BY executed_at DESC LIMIT 5;"
   ```
