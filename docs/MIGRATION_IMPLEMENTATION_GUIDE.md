...
run_database_migrations() {
    ...
    if npm run migrate 2>&1 | tee -a "$LOG_FILE"; then
        ...
        print_error "  1. Restore database: PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db < BACKUP_PATH/database.sql"
...
```yaml
      - name: 📊 Deployment Summary
        ...
        run: |
          ...
          echo "- [Production Site](https://crm.allcheckservices.com)" >> $GITHUB_STEP_SUMMARY
          echo "- [API Health](https://crm.allcheckservices.com/health)" >> $GITHUB_STEP_SUMMARY
          echo "- [Mobile App](https://crm.allcheckservices.com/mobile/)" >> $GITHUB_STEP_SUMMARY
```
...
ssh -p 2232 root@49.50.119.155 << 'EOF'
  PGPASSWORD=acs_password psql -h localhost -p 5432 -U acs_user -d acs_db -c "
    SELECT EXISTS (
      SELECT 1 FROM information_schema.tables 
...
# 3. Restore database
PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db < "$LATEST_BACKUP/database.sql"
