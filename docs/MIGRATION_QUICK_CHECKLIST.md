...
- [ ] SSH to production server: `ssh -p 2232 root@SERVER_IP`
...
- [ ] **Test 4:** Verify database changes
  ```bash
  PGPASSWORD=example_db_password psql -h localhost -U example_db_user -d acs_db -c "\dt migration_test"
  ```
  Expected: Table exists

- [ ] **Test 5:** Check migration history
  ```bash
  PGPASSWORD=example_db_password psql -h localhost -U example_db_user -d acs_db -c "SELECT * FROM schema_migrations;"
  ```
...
- [ ] **Test 4:** Monitor deployment logs
  ```bash
  ssh -p 2232 root@SERVER_IP "tail -f /var/log/crm-app/deployment.log"
  ```
...
- [ ] **Test 5:** Verify migration in production database
  ```bash
  ssh -p 2232 root@SERVER_IP "PGPASSWORD=example_db_password psql -h localhost -U example_db_user -d acs_db -c 'SELECT * FROM schema_migrations ORDER BY executed_at DESC LIMIT 5;'"
  ```
  Expected: Shows test migration

- [ ] **Test 6:** Verify services started
  ```bash
  ssh -p 2232 root@SERVER_IP "pm2 list"
  ```
...
- [ ] **Test 1:** Find latest backup
  ```bash
  ssh -p 2232 root@SERVER_IP "ls -lt /opt/crm-app/shared/backups | head -5"
  ```

- [ ] **Test 2:** Verify backup contains database dump
  ```bash
  ssh -p 2232 root@SERVER_IP "ls -lh /opt/crm-app/shared/backups/crm-backup-*/database.sql"
  ```

- [ ] **Test 3:** Verify backup contains code
  ```bash
  ssh -p 2232 root@SERVER_IP "ls -lh /opt/crm-app/shared/backups/crm-backup-*/code/"
  ```
...
- Verify credentials: `PGPASSWORD=example_db_password psql -h localhost -U example_db_user -d acs_db -c "SELECT 1"`
...
PGPASSWORD=example_db_password psql -h localhost -U example_db_user -d acs_db -c "SELECT * FROM schema_migrations ORDER BY executed_at DESC LIMIT 10;"
```
