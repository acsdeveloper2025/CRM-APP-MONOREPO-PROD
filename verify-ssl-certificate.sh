...
# 🔒 SSL Certificate Verification Script for example.com
...
print_header "🔒 SSL Certificate Verification for example.com"
...
if sudo nginx -t >/dev/null 2>&1; then
    ...
    if grep -q "/etc/ssl/certs/crm-1year.crt" /etc/nginx/sites-available/example.com; then
...
if curl -k -s -I https://localhost >/dev/null 2>&1; then
    ...
    cert_details=$(echo | openssl s_client -connect localhost:443 -servername example.com 2>/dev/null | openssl x509 -noout -dates 2>/dev/null)
...
print_info "🌐 Domain Coverage: example.com, *.example.com"
print_info "🔢 IP Coverage: PUBLIC_STATIC_IP"
