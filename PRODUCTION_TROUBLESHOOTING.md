...
# Test connection
PGPASSWORD=acs_password psql -h localhost -U acs_user -d acs_db
```
...
sudo -u postgres psql -c "CREATE USER acs_user WITH PASSWORD 'acs_password';"
sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE acs_db TO acs_user;"
```
...
openssl x509 -in /etc/letsencrypt/live/crm.allcheckservices.com/cert.pem -text -noout | grep "Not After"
```
