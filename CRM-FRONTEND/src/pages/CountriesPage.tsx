...
export function CountriesPage() {
  ...
  const { data: listData, isLoading } = useQuery({
    queryKey: ['countries', queryArgs],
    queryFn: () => locationsService.getCountries(queryArgs),
  });

  const { data: statsData } = useQuery({
    queryKey: ['country-stats'],
    queryFn: () => locationsService.getCountriesStats(),
  });
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const response = await locationsService.exportCountries({
      ...
      a.href = url;
      a.download = `countries_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      logger.error('Countries export failed', err);
  ...
  const filterContent = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <div className="space-y-1">
        <Label htmlFor="country-status-filter">Status</Label>
      ...
      <div className="space-y-1">
        <Label htmlFor="country-date-from">Date From</Label>
        <Input
          id="country-date-from"
          type="date"
          value={dateFrom}
      ...
      <div className="space-y-1">
        <Label htmlFor="country-date-to">Date To</Label>
        <Input
          id="country-date-to"
          type="date"
          value={dateTo}
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Disabled countries</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">With States</CardTitle>
            <MapPin className="h-4 w-4 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.withStatesCount}</div>
            <p className="text-xs text-muted-foreground">Has ≥1 state</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="p-4 sm:p-6 space-y-4">
          ...
          <CountriesTable data={listData?.data || []} isLoading={isLoading} />

          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
            <div className="text-sm text-muted-foreground">
              {total > 0
                ? `Showing ${listData?.data?.length || 0} of ${total} countries`
                : 'No countries to show'}
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor="country-page-size" className="text-sm text-muted-foreground">
                  Rows
                </Label>
                <Select value={String(pageSize)} onValueChange={(v) => updateParam('pageSize', v)}>
                  <SelectTrigger id="country-page-size" className="w-[80px] h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
