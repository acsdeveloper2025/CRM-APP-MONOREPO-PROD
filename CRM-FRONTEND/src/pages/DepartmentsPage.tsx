...
import { Plus, Building2, CheckCircle, XCircle, Calendar, Download, Users } from 'lucide-react';
...
export function DepartmentsPage() {
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const response = await departmentsService.exportDepartments({
      ...
      logger.error('Departments export failed', err);
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <div className="space-y-1">
        ...
        <Select value={status} onValueChange={(v) => updateParam('status', v)}>
          <SelectTrigger id="dept-status-filter">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1">
        <Label htmlFor="dept-sort">Sort by</Label>
        <Select value={sort} onValueChange={(v) => updateParam('sort', v)}>
          <SelectTrigger id="dept-sort">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {SORT_OPTIONS.map((o) => (
      ...
      <div className="space-y-1">
        <Label htmlFor="dept-date-from">Date From</Label>
        <Input
          id="dept-date-from"
          type="date"
          value={dateFrom}
      ...
      <div className="space-y-1">
        <Label htmlFor="dept-date-to">Date To</Label>
        <Input
          id="dept-date-to"
          type="date"
          value={dateTo}
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage organisational departments used by user assignments.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Disabled departments</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.withUsersCount}</div>
            <p className="text-xs text-muted-foreground">Have ≥1 user assigned</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="p-4 sm:p-6 space-y-4">
          ...
          <DepartmentsTable data={listData?.data || []} isLoading={isLoading} />

          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
            <div className="text-sm text-muted-foreground">
              {total > 0
                ? `Showing ${listData?.data?.length || 0} of ${total} departments`
                : 'No departments to show'}
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor="dept-page-size" className="text-sm text-muted-foreground">
                  Rows
                </Label>
                <Select value={String(pageSize)} onValueChange={(v) => updateParam('pageSize', v)}>
                  <SelectTrigger id="dept-page-size" className="w-[80px] h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <CreateDepartmentDialog open={showCreate} onOpenChange={setShowCreate} />
    </div>
  );
}
