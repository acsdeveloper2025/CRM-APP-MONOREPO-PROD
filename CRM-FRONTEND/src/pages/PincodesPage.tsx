...
import { Plus, Upload, MapPin, CheckCircle, XCircle, Calendar, Download } from 'lucide-react';
...
type SortValue = 'code_asc' | 'code_desc' | 'created_desc' | 'created_asc' | 'updated_desc';
...
const SORT_OPTIONS: Array<{
  ...
  { value: 'code_asc', label: 'Code A → Z', sortBy: 'code', sortOrder: 'asc' },
  { value: 'code_desc', label: 'Code Z → A', sortBy: 'code', sortOrder: 'desc' },
...
export function PincodesPage() {
  ...
  const [showCascadingCreate, setShowCascadingCreate] = useState(false);
  ...
  }, [debouncedSearchValue, status, sort, cityId, pageSize, dateFrom, dateTo]);
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: () => locationsService.getPincodesStats(),
  });

  const { data: citiesData } = useQuery({
    queryKey: ['cities-for-filter'],
    queryFn: () => locationsService.getCities({ limit: 500 }),
  });
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const response = await locationsService.exportPincodes({
        search: queryArgs.search,
        isActive: queryArgs.isActive,
        cityId: queryArgs.cityId,
      ...
      a.href = url;
      a.download = `pincodes_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      logger.error('Pincodes export failed', err);
  ...
  const filterContent = (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <div className="space-y-1">
        <Label htmlFor="pincode-status-filter">Status</Label>
        <Select value={status} onValueChange={(v) => updateParam('status', v)}>
          <SelectTrigger id="pincode-status-filter">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1">
        <Label htmlFor="pincode-city-filter">City</Label>
        <Select value={cityId} onValueChange={(v) => updateParam('city', v)}>
          <SelectTrigger id="pincode-city-filter">
      ...
      <div className="space-y-1">
        <Label htmlFor="pincode-sort">Sort by</Label>
        <Select value={sort} onValueChange={(v) => updateParam('sort', v)}>
          <SelectTrigger id="pincode-sort">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
            {SORT_OPTIONS.map((o) => (
      ...
      <div className="space-y-1">
        <Label htmlFor="pincode-date-from">Date From</Label>
        <Input
          id="pincode-date-from"
          type="date"
          value={dateFrom}
      ...
      <div className="space-y-1">
        <Label htmlFor="pincode-date-to">Date To</Label>
        <Input
          id="pincode-date-to"
          type="date"
          value={dateTo}
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Pincodes</h1>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">All pincodes</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Disabled pincodes</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">With Areas</CardTitle>
            <MapPin className="h-4 w-4 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.withAreasCount}</div>
            <p className="text-xs text-muted-foreground">Has ≥1 area</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardContent className="p-4 sm:p-6 space-y-4">
          <UnifiedSearchFilterLayout
            ...
            searchPlaceholder="Search pincodes by code, city, state, or area..."
            ...
            actions={
              <div className="flex flex-wrap gap-2">
                ...
                <Button size="sm" onClick={() => setShowCascadingCreate(true)}>
                  <Plus className="h-4 w-4 mr-2" />
                  Add Pincode
                </Button>
              </div>
          ...
          <PincodesTable data={listData?.data || []} isLoading={isLoading} />

          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
            <div className="text-sm text-muted-foreground">
              {total > 0
                ? `Showing ${listData?.data?.length || 0} of ${total} pincodes`
                : 'No pincodes to show'}
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Label htmlFor="pincode-page-size" className="text-sm text-muted-foreground">
                  Rows
                </Label>
                <Select value={String(pageSize)} onValueChange={(v) => updateParam('pageSize', v)}>
                  <SelectTrigger id="pincode-page-size" className="w-[80px] h-8">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
      ...
      <CreatePincodeDialog open={showCreate} onOpenChange={setShowCreate} />
      <CascadingCreatePincodeDialog
        open={showCascadingCreate}
        onOpenChange={setShowCascadingCreate}
      />
