...
function transformTaskData(task: any): VerificationTask {
  return {
    ...
    verificationTypeId: task.verificationType?.id || task.verification_type_id,
    verificationTypeName: task.verificationType?.name || task.verification_type_name,
    taskTitle: task.taskTitle || task.task_title,
    taskDescription: task.taskDescription || task.task_description,
    priority: task.priority,
    assignedTo: task.assignedTo?.id || task.assigned_to,
    assignedToName: task.assignedTo?.name || task.assigned_to_name,
    assignedToEmployeeId: task.assignedTo?.employeeId || task.assigned_to_employee_id,
    assignedBy: task.assignedBy?.id || task.assigned_by,
    assignedByName: task.assignedBy?.name || task.assigned_by_name,
    assignedAt: task.assignedAt || task.assigned_at,
    status: task.status,
    verificationOutcome: task.verificationOutcome || task.verification_outcome,
    rateTypeId: task.rateType?.id || task.rate_type_id,
    rateTypeName: task.rateType?.name || task.rate_type_name,
    estimatedAmount: task.estimatedAmount || task.estimated_amount,
    actualAmount: task.actualAmount || task.actual_amount,
    address: task.address,
    pincode: task.pincode,
    latitude: task.latitude,
    longitude: task.longitude,
    trigger: task.trigger,
    applicantType: task.applicantType || task.applicant_type,
    documentType: task.documentType || task.document_type,
    documentNumber: task.documentNumber || task.document_number,
    documentDetails: task.documentDetails || task.document_details,
    estimatedCompletionDate: task.estimatedCompletionDate || task.estimated_completion_date,
    startedAt: task.startedAt || task.started_at,
    completedAt: task.completedAt || task.completed_at,
    commissionStatus: task.commissionStatus || task.commission_status,
    calculatedCommission: task.calculatedCommission || task.calculated_commission,
    createdAt: task.createdAt || task.created_at,
    updatedAt: task.updatedAt || task.updated_at,
    createdBy: task.createdBy || task.created_by,
    caseNumber: task.caseNumber || task.case_number,
    customerName: task.customerName || task.customer_name
  };
}
