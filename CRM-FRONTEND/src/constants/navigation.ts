import type { Role } from '@/types/auth';
import {
  LayoutDashboard,
  FileText,
  Building2,
  MapPin,
  Receipt,
  BarChart3,
  UserCog,
  Users,
  CheckSquare,
  Settings,
  Shield,
  Plus,
  Clock,
  DollarSign,
  PlayCircle,
  List,
  XCircle,
  Copy,
  FileCheck,
  ClipboardList,
  Stethoscope,
  Radio,
  Briefcase,
  type LucideIcon,
} from 'lucide-react';

export interface NavigationItem {
  id: string;
  label: string;
  href: string;
  icon: LucideIcon;
  roles?: Role[]; // Made optional for backward compatibility
  permissionCode?: string;
  permission?: {
    resource: string;
    action: string;
  };
  children?: NavigationItem[];
}

export const navigationItems: NavigationItem[] = [
  {
    id: 'dashboard',
    label: 'Dashboard',
    href: '/dashboard',
    icon: LayoutDashboard,
    permissionCode: 'page.dashboard',
    permission: { resource: 'cases', action: 'read' }, // Using cases permission for dashboard access
  },
  {
    id: 'cases',
    label: 'Case Management',
    href: '/case-management',
    icon: FileText,
    permissionCode: 'page.cases',
    permission: { resource: 'cases', action: 'read' },
    children: [
      {
        id: 'cases-new',
        label: 'Create New Case',
        href: '/case-management/create-new-case',
        icon: Plus,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'create' },
      },
      {
        id: 'cases-list',
        label: 'All Cases',
        href: '/case-management/all-cases',
        icon: FileText,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
      {
        id: 'cases-in-progress',
        label: 'In Progress Cases',
        href: '/case-management/in-progress',
        icon: FileText,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
      {
        id: 'cases-submitted-for-review',
        label: 'Submitted for Review',
        href: '/case-management/submitted-for-review',
        icon: ClipboardList,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
      {
        id: 'cases-completed',
        label: 'Completed Cases',
        href: '/case-management/completed',
        icon: FileText,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
    ],
  },
  {
    id: 'tasks',
    label: 'Task Management',
    href: '/task-management/all-tasks',
    icon: CheckSquare,
    permissionCode: 'page.tasks',
    permission: { resource: 'tasks', action: 'read' },
    children: [
      {
        id: 'tasks-pending',
        label: 'Pending Tasks',
        href: '/task-management/pending-tasks',
        icon: Clock,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-revoked',
        label: 'Revoke Tasks',
        href: '/task-management/revoke-tasks',
        icon: XCircle,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-in-progress',
        label: 'In Progress Tasks',
        href: '/task-management/in-progress-tasks',
        icon: PlayCircle,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-revisit',
        label: 'Revisit Tasks',
        href: '/task-management/revisit-tasks',
        icon: Copy,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-awaiting-review',
        label: 'Awaiting Review',
        href: '/task-management/awaiting-review',
        icon: ClipboardList,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-completed',
        label: 'Completed Tasks',
        href: '/task-management/completed-tasks',
        icon: CheckSquare,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tat-monitoring',
        label: 'TAT Monitoring',
        href: '/task-management/tat-monitoring',
        icon: Clock,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
      {
        id: 'tasks-all',
        label: 'All Tasks',
        href: '/task-management/all-tasks',
        icon: List,
        permissionCode: 'page.tasks',
        permission: { resource: 'tasks', action: 'read' },
      },
    ],
  },
  {
    // Mandatory backend review: field verifications submitted by field
    // executives awaiting a backend user's official Backend Final Result.
    id: 'backend-review',
    label: 'Backend Review',
    href: '/backend-review',
    icon: ClipboardList,
    permissionCode: 'field_review.complete',
    permission: { resource: 'tasks', action: 'read' },
  },
  {
    // 2026-06-02: two user-facing states (Pending / Completed) + All. The
    // per-status pages reuse KYCDashboardPage with a defaultStatus prop.
    id: 'kyc-verification',
    label: 'KYC Verification',
    href: '/kyc-verification/all-kyc',
    icon: FileCheck,
    permissionCode: 'page.kyc',
    permission: { resource: 'kyc', action: 'read' },
    children: [
      {
        id: 'kyc-pending',
        label: 'Pending KYC',
        href: '/kyc-verification/pending-kyc',
        icon: Clock,
        permissionCode: 'page.kyc',
        permission: { resource: 'kyc', action: 'read' },
      },
      {
        id: 'kyc-completed',
        label: 'Completed KYC',
        href: '/kyc-verification/completed-kyc',
        icon: CheckSquare,
        permissionCode: 'page.kyc',
        permission: { resource: 'kyc', action: 'read' },
      },
      {
        id: 'kyc-all',
        label: 'All KYC',
        href: '/kyc-verification/all-kyc',
        icon: List,
        permissionCode: 'page.kyc',
        permission: { resource: 'kyc', action: 'read' },
      },
    ],
  },
  {
    id: 'data-entry-management',
    label: 'Data Entry Management',
    href: '/data-entry-management/data-entry',
    icon: ClipboardList,
    permissionCode: 'page.cases',
    permission: { resource: 'cases', action: 'read' },
    children: [
      {
        id: 'data-entry-dashboard',
        label: 'Data Entry',
        href: '/data-entry-management/data-entry',
        icon: ClipboardList,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
      {
        id: 'data-entry-mis',
        label: 'Data Entry MIS',
        href: '/data-entry-management/data-entry-mis',
        icon: BarChart3,
        permissionCode: 'page.cases',
        permission: { resource: 'cases', action: 'read' },
      },
    ],
  },
  {
    id: 'dedupe',
    label: 'Dedupe',
    href: '/dedupe',
    icon: Copy,
    permissionCode: 'page.cases',
    permission: { resource: 'cases', action: 'read' },
  },
  {
    id: 'operations',
    label: 'Operations',
    href: '#',
    icon: Radio,
    permissionCode: 'page.field_monitoring',
    children: [
      {
        id: 'field-monitoring',
        label: 'Field Monitoring',
        href: '/operations/field-monitoring',
        icon: MapPin,
        permissionCode: 'page.field_monitoring',
      },
    ],
  },
  {
    id: 'billing',
    label: 'Billing & Commission',
    href: '/billing-and-commission',
    icon: Receipt,
    permissionCode: 'page.billing',
    permission: { resource: 'billing', action: 'read' },
    children: [
      {
        id: 'invoices',
        label: 'Invoices',
        href: '/billing-and-commission/invoices',
        icon: Receipt,
        permissionCode: 'page.billing',
        permission: { resource: 'billing', action: 'read' },
      },
      {
        id: 'commission-calculations',
        label: 'Commission Calculations',
        href: '/billing-and-commission/commissions/calculations',
        icon: BarChart3,
        permissionCode: 'page.billing',
        permission: { resource: 'commissions', action: 'read' },
      },
      {
        id: 'commission-statistics',
        label: 'Commission Statistics',
        href: '/billing-and-commission/commissions/statistics',
        icon: BarChart3,
        permissionCode: 'page.billing',
        permission: { resource: 'commissions', action: 'read' },
      },
      {
        id: 'commission-management',
        label: 'Commission Management',
        href: '/billing-and-commission/commission-management',
        icon: DollarSign,
        permissionCode: 'page.billing',
        permission: { resource: 'commissions', action: 'read' },
      },
    ],
  },
  {
    id: 'reports',
    label: 'Reports & MIS',
    href: '/reports-and-mis',
    icon: BarChart3,
    permissionCode: 'page.reports',
    permission: { resource: 'reports', action: 'read' },
    children: [
      {
        id: 'analytics-overview',
        label: 'Analytics Overview',
        href: '/reports-and-mis/analytics-dashboard/overview',
        icon: BarChart3,
        permissionCode: 'page.analytics',
        permission: { resource: 'analytics', action: 'read' },
      },
      {
        id: 'analytics-cases',
        label: 'Cases Analytics',
        href: '/reports-and-mis/analytics-dashboard/cases',
        icon: BarChart3,
        permissionCode: 'page.analytics',
        permission: { resource: 'analytics', action: 'read' },
      },
      {
        id: 'analytics-tasks',
        label: 'Tasks Analytics',
        href: '/reports-and-mis/analytics-dashboard/tasks',
        icon: BarChart3,
        permissionCode: 'page.analytics',
        permission: { resource: 'analytics', action: 'read' },
      },
      {
        id: 'analytics-agents',
        label: 'Agent Performance',
        href: '/reports-and-mis/analytics-dashboard/agents',
        icon: BarChart3,
        permissionCode: 'page.analytics',
        permission: { resource: 'analytics', action: 'read' },
      },
      {
        id: 'mis-dashboard',
        label: 'MIS Dashboard',
        href: '/reports-and-mis/mis-dashboard',
        icon: BarChart3,
        permissionCode: 'page.analytics',
        permission: { resource: 'analytics', action: 'read' },
      },
    ],
  },
  {
    id: 'clients',
    label: 'Client Management',
    href: '/client-management',
    icon: Building2,
    permissionCode: 'page.masterdata',
    permission: { resource: 'clients', action: 'read' },
    children: [
      {
        id: 'clients-list',
        label: 'Clients',
        href: '/client-management/clients',
        icon: Building2,
        permissionCode: 'page.masterdata',
        permission: { resource: 'clients', action: 'read' },
      },
      {
        id: 'products',
        label: 'Products',
        href: '/client-management/products',
        icon: Settings,
        permissionCode: 'page.masterdata',
        permission: { resource: 'products', action: 'read' },
      },
      {
        id: 'verification-types',
        label: 'Verification Types',
        href: '/client-management/verification-types',
        icon: CheckSquare,
        permissionCode: 'page.masterdata',
        permission: { resource: 'verificationTypes', action: 'read' },
      },
      {
        id: 'document-types',
        label: 'Document Types',
        href: '/client-management/document-types',
        icon: FileText,
        permissionCode: 'page.masterdata',
        permission: { resource: 'documentTypes', action: 'read' },
      },
      {
        id: 'data-entry-templates',
        label: 'Data Entry Templates',
        href: '/client-management/data-entry-templates',
        icon: FileText,
        permissionCode: 'page.settings',
        permission: { resource: 'settings', action: 'read' },
      },
      {
        id: 'report-templates',
        label: 'Report Templates',
        href: '/client-management/report-templates',
        icon: FileText,
        // Gated on the manage permission so roles holding this perm (SUPER_ADMIN
        // + MANAGER today) see the menu item without also needing page.settings.
        // The route itself uses the same guard.
        permissionCode: 'report_template.manage',
        permission: { resource: 'settings', action: 'read' },
      },
    ],
  },
  {
    id: 'rate-management',
    label: 'Rate Management',
    href: '/rate-management',
    icon: DollarSign,
    permissionCode: 'page.masterdata',
    permission: { resource: 'rateManagement', action: 'read' },
    children: [
      // Phase 6 (Rate Mgmt refactor 2026-05-10): order matches dependency
      // chain — Master → Eligibility → Pricing → Geography → KYC → Reports.
      {
        id: 'rate-types',
        label: 'Rate Types',
        href: '/rate-management/rate-types',
        icon: DollarSign,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
      {
        id: 'rate-type-assignments',
        label: 'Rate Type Assignment',
        href: '/rate-management/rate-type-assignment',
        icon: Settings,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
      {
        id: 'rate-amounts',
        label: 'Rate Amounts',
        href: '/rate-management/rate-amounts',
        icon: DollarSign,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
      {
        id: 'service-zones',
        label: 'Service Zone Rules',
        href: '/rate-management/service-zone-rules',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
      {
        id: 'kyc-rates',
        label: 'KYC Rates',
        href: '/rate-management/kyc-rates',
        icon: FileText,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
      {
        id: 'rate-report',
        label: 'Rate Report',
        href: '/rate-management/rate-report',
        icon: BarChart3,
        permissionCode: 'page.masterdata',
        permission: { resource: 'rateManagement', action: 'read' },
      },
    ],
  },
  {
    id: 'locations',
    label: 'Location Management',
    href: '/location-management',
    icon: MapPin,
    permissionCode: 'page.masterdata',
    permission: { resource: 'locations', action: 'read' },
    children: [
      {
        id: 'countries',
        label: 'Countries',
        href: '/location-management/countries',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'locations', action: 'read' },
      },
      {
        id: 'states',
        label: 'States',
        href: '/location-management/states',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'locations', action: 'read' },
      },
      {
        id: 'cities',
        label: 'Cities',
        href: '/location-management/cities',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'locations', action: 'read' },
      },
      {
        id: 'pincodes',
        label: 'Pincodes',
        href: '/location-management/pincodes',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'locations', action: 'read' },
      },
      {
        id: 'areas',
        label: 'Areas',
        href: '/location-management/areas',
        icon: MapPin,
        permissionCode: 'page.masterdata',
        permission: { resource: 'locations', action: 'read' },
      },
    ],
  },
  {
    id: 'user-management',
    label: 'User Management',
    href: '#',
    icon: UserCog,
    permissionCode: 'page.users',
    permission: { resource: 'users', action: 'read' },
    children: [
      {
        id: 'users',
        label: 'Users',
        href: '/user-management/users',
        icon: Users,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'users-activities',
        label: 'User Activity',
        href: '/user-management/user-activity',
        icon: List,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'users-sessions',
        label: 'User Sessions',
        href: '/user-management/user-sessions',
        icon: Clock,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'departments',
        label: 'Departments',
        href: '/user-management/departments',
        icon: Building2,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'designations',
        label: 'Designations',
        href: '/user-management/designations',
        icon: Briefcase,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'revoke-reasons',
        label: 'Revoke Reasons',
        href: '/user-management/revoke-reasons',
        icon: XCircle,
        permissionCode: 'page.users',
        permission: { resource: 'users', action: 'read' },
      },
      {
        id: 'rbac-admin',
        label: 'RBAC Administration',
        href: '/user-management/rbac-administration',
        icon: Shield,
        permissionCode: 'page.rbac',
      },
    ],
  },
  // Form Viewer removed — unused (RBAC audit 2026-04-16)
  // Security & UX removed — was 100% demo/showcase page, theme toggle duplicated in Header (audit 2026-05-20)
  {
    id: 'system-health',
    label: 'System Health',
    href: '/admin/system-health',
    icon: Stethoscope,
    permissionCode: 'system.health',
    permission: { resource: 'settings', action: 'read' },
  },
];
