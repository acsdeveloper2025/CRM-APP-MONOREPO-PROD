...
export function FormFieldViewer({ field, readonly = true, onChange }: FormFieldViewerProps) {
  ...
  const renderReadOnlyField = () => {
    switch (field.type) {
      ...
      case 'textarea':
        return (
          <div className="text-sm whitespace-pre-wrap bg-muted/70 dark:bg-card/50 rounded-md p-3 min-h-20">
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`${readonly ? 'bg-muted/50 dark:bg-card/40 rounded-md p-3' : ''}`}>
        {renderFieldValue()}
      </div>
