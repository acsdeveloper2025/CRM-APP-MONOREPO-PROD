...
export function CountryDetailsDialog({ country, open, onOpenChange }: CountryDetailsDialogProps) {
  ...
  const getContinentColor = (continent: string) => {
    const colors: Record<string, string> = {
      ...
      Antarctica: 'bg-muted text-foreground dark:bg-card/60 dark:text-muted-foreground',
    };
    return (
      colors[continent] || 'bg-muted text-foreground dark:bg-card/60 dark:text-muted-foreground'
    );
  };
