...
export const FieldMonitoringSummaryCards = React.memo(function FieldMonitoringSummaryCards({ stats, }: FieldMonitoringSummaryCardsProps) {
    return (<Grid min={220}>
      {items.map((item) => {
            const Icon = item.icon;
            return (<Card key={item.key} {...{ className: "ui-stat-card" }}>
            <Stack gap={3}>
