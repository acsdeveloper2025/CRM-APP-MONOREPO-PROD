 ...
 * ```
 */
export const UnifiedSearchFilterLayout: React.FC<UnifiedSearchFilterLayoutProps> = ({ searchValue, onSearchChange, onSearchClear, isSearchLoading = false, searchPlaceholder = 'Search...', filterContent, hasActiveFilters = false, activeFilterCount = 0, onClearFilters, actions, showFilters = true, filtersCollapsible = true, filtersDefaultCollapsed = false, className, variant = 'stacked', }) => {
    ...
    if (variant === 'inline') {
        ...
        <div {...{ className: "flex flex-col gap-4 lg:flex-row" }}>
          ...
          <div {...{ className: "min-w-0 flex-1" }}>
            <UnifiedSearchInput value={searchValue} onChange={onSearchChange} onClear={onSearchClear} isLoading={isSearchLoading} placeholder={searchPlaceholder}/>
          </div>

          {/* Filter Toggle and Actions */}
          <div {...{ className: "flex flex-wrap items-center gap-2" }}>
            {filterContent && showFilters && (<Button variant={hasActiveFilters ? 'default' : 'outline'} size="default" onClick={() => setShowFilterPanel(!showFilterPanel)} {...{ className: "gap-2" }}>
                <Filter {...{ className: "h-4 w-4" }}/>
                Filters
                {activeFilterCount > 0 && (<span {...{ className: "ml-1 rounded-full bg-background px-2 py-0.5 text-xs font-semibold" }}>
                    {activeFilterCount}
                  </span>)}
              </Button>)}
        ...
        {/* Filter Panel */}
        {filterContent && showFilters && showFilterPanel && (<UnifiedFilterPanel hasActiveFilters={hasActiveFilters} activeFilterCount={activeFilterCount} onClearFilters={onClearFilters} collapsible={filtersCollapsible} defaultCollapsed={false}>
            {filterContent}
          </UnifiedFilterPanel>)}
      </div>);
    }
    ...
    return (<div {...{ className: cn('space-y-4', className) }}>
      ...
      {/* Filter Panel */}
      {filterContent && showFilters && (<UnifiedFilterPanel hasActiveFilters={hasActiveFilters} activeFilterCount={activeFilterCount} onClearFilters={onClearFilters} collapsible={filtersCollapsible} defaultCollapsed={filtersDefaultCollapsed}>
          {filterContent}
        </UnifiedFilterPanel>)}
    </div>);
};
...
export const SearchOnlyLayout: React.FC<SearchOnlyLayoutProps> = ({ searchValue, onSearchChange, onSearchClear, isSearchLoading = false, searchPlaceholder = 'Search...', actions, className, }) => {
